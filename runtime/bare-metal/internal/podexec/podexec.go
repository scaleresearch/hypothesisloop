...
const (
	...
	LabelDeadlineUnix    = workloadkeys.DeadlineUnix
...
type Config struct {
	RegistryURL string
	// JobDeadlineMultiplier/MinJobDeadlineSeconds/DefaultTerminationGracePeriodSeconds/
	// MaxTerminationGracePeriodSeconds mirror k8sexec.Config exactly — see there for meaning.
...
func New(cfg Config) (*Executor, error) {
	...
	if cfg.JobDeadlineMultiplier <= 0 || cfg.MinJobDeadlineSeconds <= 0 || cfg.DefaultTerminationGracePeriodSeconds <= 0 || cfg.MaxTerminationGracePeriodSeconds <= 0 {
		return nil, fmt.Errorf("podexec: retry, deadline, and termination settings must be positive")
	}
	if cfg.ScratchDir == "" {
	...
	return &Executor{
		...
		jobDeadlineMultiplier:                cfg.JobDeadlineMultiplier,
		minJobDeadlineSeconds:                int64(cfg.MinJobDeadlineSeconds),
