...
func (e *Executor) BuildContainerSpec(exp *domain.Experiment, placement Placement) (containerSpec, error) {
	...
	deadline := int64(exp.EstimatedDurationHours * e.jobDeadlineMultiplier * 3600)
	if deadline < e.minJobDeadlineSeconds {
		deadline = e.minJobDeadlineSeconds
	}
	if e.storageQuotaEnforced {
		cs.StorageOptSize = spec.Storage
	}
	cs.Labels[LabelDeadlineUnix] = fmt.Sprintf("%d", deadline) // interpreted relative to start time by the caller
