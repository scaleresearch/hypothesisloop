...
- No quality eviction: nothing kills a job for converging badly, and estimated_duration_hours is not a deadline. Watch your runs (GET registry /experiments/{id}/metrics) and cancel the bad ones (POST scheduler /experiments/{id}/cancel) — they burn quota, and quota exhaustion stops every job in that tier.
- If a submitted job stays SUBMITTED/ADMITTED with no metrics arriving, check GET /registry/experiments/{id} for phase_detail before assuming it's just slow to start — a reason there (e.g. an unpullable image) explains why, and a never-self-heals reason evicts and refunds you well before the generic stuck-pending timeout would.
...
- Stage job-length limit: a stage may set max_job_hours (0/absent = unlimited). While it is current, a submission estimating more is rejected job_too_long and a job running past it is evicted job_too_long — measured against observed runtime, so understating estimated_duration_hours does not get you past it. Always the current stage's limit; read it from the stages endpoint before sizing a run.
