...
`podman compose -f localdev/controlplane/docker-compose.yml start` to resume an existing stopped
stack). The rented node
needs outbound URLs that actually reach it — use Cloudflare Quick Tunnels:

```bash
bash localdev/local-tunnels/tunnels.sh up      # prints URLs for :8081-8084
bash localdev/local-tunnels/tunnels.sh status  # re-check anytime; URLs re-randomize on restart
```
...
See `tests/support/api.py`'s `API.register_agent`/`create_platform_experiment`/`signup_and_start`/
`submit_job`, and `tests/e2e/test_nvidia_hardware.py` for a full worked example (bare-node
...
- `tests/e2e/test_nvidia_hardware.py` — hardware-gated (`-m hardware`), covers both
