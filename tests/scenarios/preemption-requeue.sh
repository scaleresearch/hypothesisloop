...
# fraction of budget_accelerator_hours consumed (domain.Phase1ExploreFraction — the per-PE
...
# testing. A100 is an expensive accelerator type (high acch_rate) and accelerator_count=4 burst jobs, so under
...
wait_until "burst jobs admitted onto $ACCELERATOR_TYPE" 30 1 burst_jobs_admitted \
...
if [[ -n "$PREEMPTED" ]]; then
  ...
  # Preempting one 4-accelerator burst job only frees 4 of the A100 node's 8 accelerators; the 1-accelerator guaranteed
  ...
  # point ACCELERATOR_TYPE was nominally saturated by BURST_JOBS_N*ACCELERATOR_COUNT_PER_BURST=8 accelerators on an
  # 8-accelerator A100 node, so a guaranteed job requesting that same type finding nothing to preempt
...
# capacity) rather than claiming a new accelerator type, since a truly parallel `tests/run.sh` run
...
# Safety invariant sampled throughout the race window: guaranteed + running-burst accelerator count on
