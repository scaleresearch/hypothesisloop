...
# Keep estimated_duration_hours small and fixed (0.02h, ~70-90s real runtime — same as every
# other scenario's jobs): it drives the *actual* workload's wall-clock sleep loop, not just
# billing, so it can't be inflated just to make the CPU-quota math convenient. Instead the PE's
# CPU budget is sized down so a small, fast job's real cost (hours * cores) is what exhausts it.
...
CPU_SPEC="1"
BUDGET_CPU="0.08"
PE_ID=$(create_platform_experiment "cpu-quota-guard-${RUN_ID}" 0.001 1 0.90 10 "$BUDGET_CPU")
...
if [[ "$CODE" -ge 400 ]]; then
  ...
  S3=$(wait_for_status "$SECOND" "RUNNING,COMPLETED" 30 || true)
