#!/usr/bin/env bash
# acceptable_gpu_types: a job that hard-requires exactly gpu_type must still get selected a
# concrete flavor at admission time from among gpu_type + acceptable_gpu_types — not just at
...
# sitting QUEUED forever next to idle GPUs.
#
# Regression test for findings.md's P1 "acceptable_gpu_types cannot be scheduled correctly":
# scheduler capacity selection and reservation used to use only Experiment.GPUType (the
...
# so it's the only viable landing spot for the flexible job. API-only, parallel-safe (uses GPU
...
PE_ID=$(create_platform_experiment "acceptable-gpu-types-${RUN_ID}" 50.0 "${#AGENTS[@]}")
signup_and_start "$PE_ID" "${AGENTS[@]}"

echo "  ==> saturating all ${REQUESTED} capacity with a filler guaranteed job (16 is the per-job gpu_count cap — quota.max_gpu_count_per_job in openresearch.yaml — and deliberately exceeds any real ${REQUESTED} node's supply)..."
...
echo "  ==> submitting a job requesting ${REQUESTED} with acceptable_gpu_types=[${REQUESTED}, ${ALTERNATIVE}]..."
read -r CODE FLEX_ID <<< "$(submit_job_expect_code "$PE_ID" "${AGENTS[1]}" "guaranteed" "0.02" \
  "{\"gpu_type\": \"${REQUESTED}\", \"gpu_count\": 1, \"acceptable_gpu_types\": [\"${REQUESTED}\", \"${ALTERNATIVE}\"]}")"
...
if [[ "$CODE" -ge 400 ]]; then
  ...
  if [[ "$S" == "RUNNING" || "$S" == "COMPLETED" ]]; then
    ADMITTED_TYPE=$(get_field "$FLEX_ID" gpu_type)
    [[ "$ADMITTED_TYPE" == "$ALTERNATIVE" ]] \
      ...
      || fail "job reached status=$S but admitted gpu_type=$ADMITTED_TYPE (expected ${ALTERNATIVE} — requested ${REQUESTED} was saturated)"
  else
    fail "job stayed QUEUED (status=$S) with idle ${ALTERNATIVE} capacity available — admission only considered the requested flavor, not acceptable_gpu_types"
  fi
