...
#   1. fulfilling a donation actually moves guaranteed T4h from donor to requester (not just
...
DONOR_GUARANTEED_BEFORE=$(_quota_field "$PE_ID" "$DONOR" guaranteed_t4_hours)
REQUESTER_GUARANTEED_BEFORE=$(_quota_field "$PE_ID" "$REQUESTER" guaranteed_t4_hours)
...
DONOR_GUARANTEED_AFTER=$(_quota_field "$PE_ID" "$DONOR" guaranteed_t4_hours)
REQUESTER_GUARANTEED_AFTER=$(_quota_field "$PE_ID" "$REQUESTER" guaranteed_t4_hours)
...
[[ "$DONOR_DELTA" == "-$CREDITS_WANT" ]] \
  && pass "donor's guaranteed T4h debited by exactly $CREDITS_WANT (delta=$DONOR_DELTA)" \
  || fail "expected donor debited by -$CREDITS_WANT, got $DONOR_DELTA"
[[ "$REQUESTER_DELTA" == "$CREDITS_WANT" ]] \
  && pass "requester's guaranteed T4h credited by exactly $CREDITS_WANT (delta=$REQUESTER_DELTA)" \
...
DONOR_GUARANTEED_AFTER2=$(_quota_field "$PE_ID" "$DONOR" guaranteed_t4_hours)
[[ "$DONOR_GUARANTEED_AFTER2" == "$DONOR_GUARANTEED_AFTER" ]] \
  ...
  || fail "donor's guaranteed T4h changed again on re-fulfillment attempt ($DONOR_GUARANTEED_AFTER -> $DONOR_GUARANTEED_AFTER2) — double-credit"
...
CANCELLER_GUARANTEED_BEFORE=$(_quota_field "$PE_ID" "$CANCELLER" guaranteed_t4_hours)
...
CANCELLER_GUARANTEED_AFTER=$(_quota_field "$PE_ID" "$CANCELLER" guaranteed_t4_hours)
[[ "$POST_CANCEL_FULFILL_CODE" -ge 400 && "$CANCELLER_GUARANTEED_AFTER" == "$CANCELLER_GUARANTEED_BEFORE" ]] \
  ...
  || fail "cancelled donation request was fulfillable (HTTP $POST_CANCEL_FULFILL_CODE), guaranteed T4h $CANCELLER_GUARANTEED_BEFORE -> $CANCELLER_GUARANTEED_AFTER"
