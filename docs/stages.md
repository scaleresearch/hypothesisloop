...
An ordered list of stages, each two required numbers plus an optional third:
...
| **max job hours** | optional cap on a single job's length while this stage is current; **0/absent = unlimited** |
...
stages: 20/50/0.25, 30/50/1, 50/0    short jobs first, uncapped once the field has thinned
```

Validation, all rejected at creation: empty list, lengths not summing to 100, 1–8 stages,
`length > 0`, `0 ≤ evict < 100`, `max_job_hours ≥ 0`, and a non-zero `evict` on the final stage
(nothing follows it).

**Job-length cap.** While a stage is current, a submission estimating more than its
`max_job_hours` is rejected `job_too_long`, and a job that runs past it is evicted `job_too_long`.
Enforcement measures *observed* runtime, so understating `estimated_duration_hours` does not get a
job past it. The cap in force is always the current stage's, not the one frozen at submission — a
job outliving the stage that admitted it is exactly the case the cap exists to prevent.
...
- The ranking guardrails are `minSurvivorsForCut = 5` and `minSurvivorsAfterCut = 2`
  (`services/controller/stages_rank.go`). Below the first threshold `evict_pct` is inert and the
  ladder is purely a job-length schedule.
- `max_job_hours` was added after v1 (`domain.Stage.MaxJobHours`, read live via
  `PlatformExperiment.CurrentMaxJobHours`); the reject-at-submit and evict-while-running paths are
  covered by `tests/scenarios/stage-job-length.sh`.
