...
test before this script was written (KMD 2.10.0, firmware 19.11.0, sustained ~106 TFLOPS bf16
at n=2048).
...
# DRA gives this pod exactly one Tenstorrent device (accelerator_count: 1 in job.yaml) and
# only that device's /dev/tenstorrent/N node is visible inside the container — ttnn/UMD
# enumerate whatever's visible starting at local index 0, so device_id is always 0 here
# regardless of which physical card (tt-0..tt-3) the platform actually allocated; see the
# manual smoke test that established this.
DEVICE_ID = 0
