...
| Speculating onto a cluster with zero live nodes | **Allow it, blind** — `fitsLargestNode` accepts unconditionally when a cluster reports no nodes at all, instead of excluding it forever | A scaled-to-zero autoscaler cluster has no shape data of any kind to check a request against; excluding it always meant this exact case — the cluster the feature exists to grow from nothing — could never be speculated onto. A wrong guess here is bounded: `tried_clusters`/`infra_requeue_count` backoff catches it on the first failed attempt, and a per-cluster cap defaulting to one job's own footprint (below) stops it from piling up unbounded before that |
| Speculative cap default when a cluster has zero live nodes and no operator override | **One job's own accelerator footprint** | With no live data backing the blind guess above, an unset `max_speculative_accelerators` would otherwise mean unlimited jobs pile onto one unproven cluster in a single tick; defaulting to one job's footprint bounds that until the first real attempt gives the operator grounds to raise the explicit cap |
